	...
	<div class="modal-dialog" role="document">
	    <div class="modal-content">
	        ...
	        <div class="modal-body">
	            <form class="needs-validation" action="{{action('Admin\CategoryController@edit',[$category->id]) }}" method="post" enctype="multipart/form-data" name="edit_model" id="edit_model">
	                @csrf
	                <div class="form">
	                    <div class="form-group">
	                        <label for="validationCustom01" class="mb-1">Category Name<span class="required" style="color: red;">*</span></label>
	                        <input class="form-control" id="validationCustom01" type="text" name="name" value="{{$category->name}}" required="">
	                    </div>
	                    <div class="form-group">
	                        <label for="file1" class="mb-1">Category Image</label>
	                        <input class="form-control" id="file1" type="file" name="image" onchange="return imgValidation1(event)" accept=".png,.jpg,.jpeg">
	                        @if($category->image)
                           <img src="{{asset($category->image)}}" class="img-circle mt-2" width="125" height="125" alt="{{$category->name}}">
                           @endif
	                    </div>
	                    <!-- <div class="form-group">
