            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                       <form class="form-horizontal" action="{{ action('Admin\OrderManagementController@recentOrder') }}" method="get">
                          <div class="form-row">
                            ...
                            <div class="form-group d-flex align-items-end col-md-3">
                                ...
                                <a href="{{url('/admin/recent-order')}}" class="btn btn-secondary" type="button">Reset</a>
                            </div>
                          </div>
                        </form>
                    </div>
                    <div class="card-body order-datatable">
                        <table class="display jsgrid" id="basic-1">
                            ...
                            <tbody>      
                                @php 
                                      ...
                                      $sort = $recentOrderList->perPage();
                                  $count = (($recentOrderList->currentpage()-1)* $recentOrderList->perpage() + 1); 
                                @endphp                                    
                                @foreach($recentOrderList as $recentOrder)
                                <tr>
                                    <td>{{ $count++ }}</td>
                                    <td>{{ $recentOrder->order_id ?? '' }}</td>
                                    
                                    <td>{{ $recentOrder->order_amount ?? '' }} ₹</td>
                                    <td><span class="badge badge-success">{{ $recentOrder->payment_status == '1' ? 'Paid' : '' }}</span></td>
                                    <!-- <td><span class="badge badge-secondary">Cash On Delivery</span></td> -->
                                    <td>Online</td>
                                    <td>
                                        @if($recentOrder->order_status == '1' ??'')
                                    ...
                                    <td>
                                          @php 
                                            ...
                                            $orderDate = date('Y-m-d', strtotime($recentOrder->created_at));

                                          @endphp
                                          
                                          @if($today_date > $orderDate )
                                            {{ $orderDate ??'' }}
                                          @else
                                            {{ $today_date ??'' }}
                                          @endif
                                    ...
                                    <td>
                                         <a type="button" data-href="{{ action('Admin\OrderManagementController@editOrder',[$recentOrder->id]) }}" class="mr-2 edit_model" data-toggle="modal" data-target="#edit_model"><i class="fa fa-edit text-success"></i></a>

                                          <a href="{{ action('Admin\OrderManagementController@view',[$recentOrder->id] ?? '') }}" class="fa-icon fa fa-eye mr-2 "></a>
                        ...
                        {{ $recentOrderList->appends(Request::except('page'))->links() }}
                    </div>
                </div>
            </div>
        </div>
